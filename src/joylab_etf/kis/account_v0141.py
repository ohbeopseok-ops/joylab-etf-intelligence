...
class KISAccountAdapter:
    ...
    def get_balance(self) -> tuple[list[AccountPosition], AccountSummary]:
        ...
        if not response.ok:
            # 계좌번호/키/토큰은 출력하지 않고 KIS 본문 메시지만 확인
            raise RuntimeError(
                f"KIS Balance HTTP {response.status_code} "
                f"(env={self.settings.env}, tr_id={self.tr_id}): "
                f"{response.text[:800]}"
            )
        ...
        summary_row = output2[0] if isinstance(output2, list) and output2 else {}
